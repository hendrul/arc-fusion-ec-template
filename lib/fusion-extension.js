...
export const FusionContextProvider = ({ featurePack, defLang, children }) => {
  // prettier-ignore
  ...
  const strings = React.useRef(getMessages(featurePack, site, defLang)).current;
  ...
  const { [featurePack]: appProps = {} } = interpolatedSiteProperties;
  ...
  return (
    <Context.Provider value={finalFusionContext}>
      <ThemeProvider app={featurePack}>{children}</ThemeProvider>
    </Context.Provider>
  );
};
...
function getMessages(featurePack, site, lang) {
  let msgs = {};
  try {
    msgs = require(`../../i18n/${featurePack}/${lang}/${site}`).default;
  } catch (e) {
    // Los mensajes por defecto no tienen una carpeta con codigo de lenguaje
    try {
      msgs = require(`../../i18n/${featurePack}/${site}`).default;
    } catch (e2) {}
  }
  return {
    ...msgs,
