...
export const useFusionContext = () => {
  const fusionContext = React.useContext(Context);
  // Devolver el contexto fusion extendido si ya existe
  if (fusionContext) return fusionContext;
  else {
    try {
      // Devolver el contexto fusion real
      return require("fusion:context").useFusionContext();
    } catch (e) {
      console.warn(
        "No esta ejecutando sobre un contexto fusion real, se va a simular un contexto para prueba"
      );
      // Devolver el contexto fusion falso
      return require("../../mocks/fusion/fusion-context").useFusionContext();
    }
  }
};
